...
public class ShoppingProgram{
    ...
    Item[]_store = ArrayBag.readFile();
     BagInterface<Item> shoppingbag = new ArrayBag<>();
    BagInterface<Item> wishbag = new ArrayBag<>();
    ...
    System.out.println("G ->  exit");
        ...
        char command = parts[0].toUpperCase().charAt(0);
         if (!ArrayBag.VerifyCommandInput(command, parts)){
        continue;
         }
        switch (command){
        ...
        if(qtyA <= 0){System.out.println("Quantity must be greater than 0");
        break;
        }
        if(qtyA > MAX_QUANTITY){System.out.println("Max quantity to add at a time is " + MAX_QUANTITY);  
        break;
        }
        Item found = ArrayBag.findByRfid(_store, rfidA);
        if (found == null){
            System.out.println("Item with RFID " + rfidA + " does not exist within the store");
            break;
        }
        ArrayBag.printBagChoicesToAdd();
        ...
        if (rfidR <1 || rfidR > 30){System.out.println("Item with " + rfidR +" does not exist");
        break;
        }
        int qtyR = Integer.parseInt(parts[2]);
        if(qtyR <= 0){System.out.println("Quantity must be greater than 0");
        break;
        }
        if(qtyR > MAX_QUANTITY){System.out.println("Max quantity to remove at a time is " + MAX_QUANTITY);  
        break;
        }
       
        ArrayBag.printBagChoicesToRemove();
        String whichbagforremoval = scanner.nextLine().trim();
        if (whichbagforremoval.equalsIgnoreCase("S")){
        int removedAmount = 0;
        for(int i = 0; i < qtyR; i++){
        Item itemtoremove = ArrayBag.findByRfidForBag(shoppingbag, rfidR );
        ...
        int removedAmount = 0;
        for(int i = 0; i < qtyR; i++){
        Item itemtoremove = ArrayBag.findByRfidForBag(wishbag, rfidR );
        if (itemtoremove!=null){
        ...
        else {System.out.println("invalid bag");}
        break;
        case 'H':
        ArrayBag.printHelp();
        break;
        ...
        ArrayBag.PrintBagAndPrice(shoppingbag);
        break;
            
        case 'D':
        System.out.println("Shopping bag Shown Below:");
        ArrayBag.PrintBagAndPrice(shoppingbag);
        System.out.println("Checkout: Total price is $" + ArrayBag.addPrices(shoppingbag));
        System.out.println();
        break;
        ...
        ArrayBag.PrintBagAndPrice(wishbag);
        System.out.println("Checkout: Total price is $" + ArrayBag.addPrices(wishbag)+"$");
        System.out.println();
        break;
        ...
        default: System.out.println("Invalid input. Type H if you need assistance");
    }   
    }
    }
    }
